...
from models.bert_for_ner import BertSpanV2ForNer
...
class NerArgumentParser(ArgumentParser):
    ...
    def build_arguments(self):
        ...
        self.add_argument("--ltp_model_name_or_path", default="/home/louishsu/NewDisk/Garage/weights/ltp/base1.tgz", type=str)
    ...
    def _create_examples(self, data_dir, data_file, mode):
        ...
        with open(data_path, encoding="utf-8") as f:
            ...
            for sentence_counter, line in enumerate(lines):
                ...
                seglen = [len(s) for s in seg[0]]
                seg = [0] + list(accumulate(seglen))[:-1]
...
class Example2Feature:
    ...
    def _convert_example_to_feature(self, example):
        ...
        # lexicon
        seg, pos = example[1]["seg"], example[1]["pos"]

        # encode input
        inputs = self.tokenizer.encode_plus(
            text=tokens + [f"[{v}]" for v in pos],
            # text=tokens + [f"[unused1]" for v in pos],
            text_pair=None, 
        ...
        position_ids[len(tokens) + 1: len(tokens) + 1 + len(seg)] = [i + 1 for i in seg]
        inputs["position_ids"] = torch.tensor([position_ids])
...
MODEL_CLASSES = {
    "bert_span_v2_lex": (BertConfigSpanV2, BertSpanV2ForNer, BertTokenizer),
}
...
if __name__ == "__main__":
    ...
    # args = parser.parse_args_from_json(json_file="args/bert_span_v2_lex-clue_ner.json")
    ...
    # lexicon: pos tokens
    ltp = LTP(args.ltp_model_name_or_path)
    pos_tokens = [f"[{v}]" for v in ltp.pos_vocab]
    ...
    tokenizer.add_tokens(pos_tokens)
