...
from models.bert_for_ner import BertSpanV3ForNer
...
class BertConfigSpanV3(BertConfig):
...
def evaluate(args, model, processor, tokenizer, prefix=""):
    ...
    for step, batch in pbar:
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                ...
                if args.model_type.split('_')[0] in ["roberta", "xlnet"]:
                    batch["token_type_ids"] = None
            outputs = model(**batch)
            tmp_eval_loss, (logits, mask) = outputs[:2]
        if args.n_gpu > 1:
        ...
        # calculate metrics
        preds = model.span.decode_batch(logits, mask)
        ...
        labels = model.span.decode_batch(batch["labels"], mask, is_logits=False)
...
MODEL_CLASSES = {
    "bert_span_v3": (BertConfigSpanV3, BertSpanV3ForNer, BertTokenizer),
}
...
if __name__ == "__main__":
    ...
    args = parser.parse_args_from_json(json_file="args/bert_span_v3-clue_ner.json")
